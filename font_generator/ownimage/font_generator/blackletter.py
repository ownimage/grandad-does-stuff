...
class Blackletter:
    def __init__(self, fp: FontParameters):
        ...
        # strokes
        self.s_a1 = Stroke(Point(0, self.b3m - self.x3m), StrokeType.Block)
        self.s_a2 = Stroke(Point(2 * self.m, -2 * self.m), StrokeType.Block)

        self.s_b1 = Stroke(Point(0, self.b3m - fp.ascender), StrokeType.Block)
        ...
        # marks
        self.m_a1 = Mark(Point(0, self.x3m), [self.cs_a1])
        self.m_a2 = Mark(Point(2 * self.m, self.xm), [self.cs_a2])

        self.m_b1 = Mark(Point(0, self.fp.ascender), [self.cs_b1])
        self.m_b2 = Mark(Point(2 * self.m, self.xm), [self.cs_b2])

        self.m_c1 = Mark(Point(0, self.x3m), [self.cs_c1])
        self.m_c2 = Mark(Point(2 * self.m, self.xm), [self.s_a2])
    ...
    def svg(self, posn: Point, chars: str, scale: float):
        ...
        for c in chars:
            ...
            start = Point(start.x + self.glyph_widths[c], start.y)
        print(f"svg={svg}")
        return svg
