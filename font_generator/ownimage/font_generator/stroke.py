...
class Stroke(Strokeable):
    ...
    def birdfont_path(self, start: Point, fp: FontParameters, scale: float):
        ...
        paths = [
            {
                "data": f"S {p1.x:.5f},{p1.y:.5f} L {p2.x:.5f},{p2.y:.5f} L {p3.x:.5f},{p3.y:.5f} L {p4.x},{p4.y:.5f} L {p1.x:.5f},{p1.y:.5f}",
            }
        ]
        ...
        if ip1.is_empty or ip2.is_empty or ip3.is_empty or ip4.is_empty:
            ...
            if not fp.filled:
                paths += [
                    {
                        "data": f"S {p1.x:.5f},{p1.y:.5f} L {p1offset.x:.5f},{p1offset.y:.5f} L {p3offset.x:.5f},{p3offset.y:.5f} L {p3.x:.5f},{p3.y:.5f} L {p1.x:.5f},{p1.y:.5f}",
                    }
                ]
        ...
        if not fp.filled:
            paths += [
                {
                    "data": f"S {ip1.x:.5f},{ip1.y:.5f} L {ip4.x:.5f},{ip4.y:.5f} L {ip3.x:.5f},{ip3.y:.5f} L {ip2.x:.5f},{ip2.y:.5f} L {ip1.x:.5f},{ip1.y:.5f}",
                }
            ]
        return VM.add_points(start, self.vec), paths
